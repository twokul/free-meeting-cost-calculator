...
export async function fetchMeetingData(days: number = 30, hourlyRate: number = 100): Promise<{ meetings: Meeting[], error?: string }> {
  ...
  try {
    ...
    const meetings: Meeting[] = events
      .filter((event) => {
        ...
        const attendees = event.attendees?.filter(a => !a.resource && a.responseStatus !== 'declined') || [];
      ...
      .map((event) => {
        ...
        const attendeesCount = event.attendees?.filter(a => !a.resource && a.responseStatus !== 'declined').length || 1;
